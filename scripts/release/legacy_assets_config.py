from __future__ import annotations

CORE_PARTIALS = [
    '00-mobile-topbar.html',
    '01-sidebar.html',
    '02-mobile-toggle.html',
    '03-mobile-mask.html',
    '04-main-area.html',
    '24-mobile-bottombar.html',
]

CSS_SOURCES = [
    'styles/main/00-base-reset.css',
    'styles/main/01-sidebar.css',
    'styles/main/02-main-area.css',
    'styles/main/03-tab-bar.css',
    'styles/main/04-left-menu.css',
    'styles/main/05-center-notes.css',
    'styles/main/06-right-errors.css',
    'styles/main/07-stats-breadcrumbs.css',
    'styles/main/08-error-list.css',
    'styles/main/09-error-card.css',
    'styles/main/10-modal-base.css',
    'styles/main/11-claude-modal.css',
    'styles/main/12-quiz-modal.css',
    'styles/main/13-storage-warning.css',
    'styles/main/14-image-preview.css',
    'styles/main/15-image-responsive.css',
    'styles/main/16-per-question-note.css',
    'styles/main/17-dashboard.css',
    'styles/main/18-practice-history.css',
    'styles/main/19-chapter-filter.css',
    'styles/main/20-quiz-skip.css',
    'styles/main/21-chapter-stats.css',
    'styles/main/23-md-toolbar.css',
    'styles/main/24-export-modal.css',
    'styles/main/25-type-rules.css',
    'styles/main/26-difficulty-stars.css',
    'styles/main/27-bulk-actions.css',
    'styles/main/28-stats-tab.css',
    'styles/main/29-directory-management.css',
    'styles/main/30-inline-quiz.css',
    'styles/main/31-right-notes-panel.css',
    'styles/main/32-note-type-tags.css',
    'styles/main/33-note-split-editor.css',
    'styles/main/34-right-note-title-items.css',
    'styles/main/35-ai-workbench-tabs.css',
    'styles/main/37-layout-refresh-v31.css',
    'styles/main/39-print.css',
]

JS_SOURCES = [
    'modules/main/00-event-dispatcher.js',
    'modules/main/state/01a-reason-utils.js',
    'modules/main/state/01b-error-workflow-utils.js',
    'modules/main/state/01c-knowledge-search-ui.js',
    'modules/main/01-state.js',
    'modules/main/02-indexeddb.js',
    'modules/main/03-storage-usage.js',
    'modules/main/04-utils.js',
    'modules/main/persistence/04a-startup-summary.js',
    'modules/main/persistence/04b-persist-primitives.js',
    'modules/main/persistence/05e-sync-snapshots.js',
    'modules/main/persistence/05f-portable-backup.js',
    'modules/main/persistence/05g-cloud-fetch.js',
    'modules/main/persistence/05h-cloud-restore-flow.js',
    'modules/main/persistence/05i-cloud-ui.js',
    'modules/main/persistence/05j-cloud-entry-actions.js',
    'modules/main/persistence/05k-cloud-save-actions.js',
    'modules/main/persistence/05l-cloud-schedule-actions.js',
    'modules/main/persistence/05m-sync-op-queue.js',
    'modules/main/persistence/05n-sync-cursor.js',
    'modules/main/persistence/05o-load-workspace-data.js',
    'modules/main/persistence/05p-backup-payload.js',
    'modules/main/persistence/05q-apply-remote-ops.js',
    'modules/main/05-persistence.js',
    'modules/main/persistence/05a-note-sync-and-history.js',
    'modules/main/persistence/05b-cloud-bootstrap-and-schedule.js',
    'modules/main/persistence/05c-cloud-restore-origin-utils.js',
    'modules/main/persistence/05d-local-backup-ui.js',
    'modules/main/06-ai-workbench.js',
    'modules/main/image/07a-image-value-utils.js',
    'modules/main/image/07b-ocr-question-ui.js',
    'modules/main/image/07c-ocr-question-runner.js',
    'modules/main/image/07d-global-search-preview.js',
    'modules/main/image/07e-process-image-modal.js',
    'modules/main/image/07f-global-search-modal-and-filters.js',
    'modules/main/07-image-processing.js',
    'modules/main/08-initial-data.js',
    'modules/main/09-markdown.js',
    'modules/main/10-notes-panel.js',
    'modules/main/11-typed-note-tree.js',
    'modules/main/12-review-engine.js',
    'modules/main/modal/13b-quiz-pause-timer.js',
    'modules/main/modal/13c-quiz-queue-entry.js',
    'modules/main/modal/13d-quiz-session-shell.js',
    'modules/main/modal/13e-quiz-submit-flow.js',
    'modules/main/modal/13f-quiz-render-answer.js',
    'modules/main/modal/13g-quiz-review-render.js',
    'modules/main/modal/13h-random-note-candidates.js',
    'modules/main/modal/13i-random-note-practice-priority.js',
    'modules/main/modal/13j-random-note-modal-render.js',
    'modules/main/modal/13k-random-note-actions.js',
    'modules/main/modal/13l-random-question.js',
    'modules/main/13-quiz-flow.js',
    'modules/main/modal/13a-quiz-canvas-feedback.js',
    'modules/main/14-sidebar-render.js',
    'modules/main/15-filters.js',
    'modules/main/16-main-render.js',
    'modules/main/workspace/16a-workspace-actions.js',
    'modules/main/workspace/16b-workspace-list-render.js',
    'modules/main/17-error-card-render.js',
    'modules/main/workspace/17a-error-card-actions.js',
    'modules/main/18-crud-modal.js',
    'modules/main/19-import-export.js',
    'modules/main/modal/19a-import-export-core.js',
    'modules/main/modal/19b-import-parse.js',
    'modules/main/modal/19c-import-normalize.js',
    'modules/main/modal/19d-import-merge.js',
    'modules/main/modal/19e-entry-flow-ui.js',
    'modules/main/20-claude-helper.js',
    'modules/main/21-dashboard-modules.js',
    'modules/main/22-history.js',
    'modules/main/23-md-toolbar.js',
    'modules/main/24-type-detection.js',
    'modules/main/25-quick-print.js',
    'modules/main/26-export-upgrade.js',
    'modules/main/27-backup-restore.js',
    'modules/main/28-vocabulary-bank.js',
    'modules/main/29-quiz-shortcuts.js',
    'modules/main/knowledge/30a-knowledge-tree-state.js',
    'modules/main/knowledge/30b-knowledge-tree-render.js',
    'modules/main/knowledge/30c-knowledge-tree-actions.js',
    'modules/main/knowledge/30d-knowledge-tree-core.js',
    'modules/main/knowledge/30e-knowledge-tree-modal-flow.js',
    'modules/main/knowledge/30f-knowledge-tree-binding.js',
    'modules/main/knowledge/30g-knowledge-tree-dnd.js',
    'modules/main/knowledge/30u-knowledge-tree-sort-modal.js',
    'modules/main/knowledge/30h-walk-sync-notes.js',
    'modules/main/knowledge/30i-note-reference-utils.js',
    'modules/main/knowledge/30j-duplicate-merge.js',
    'modules/main/knowledge/30k-path-title-utils.js',
    'modules/main/knowledge/30l-node-collect-and-expand.js',
    'modules/main/knowledge/30m-legacy-path-normalization.js',
    'modules/main/knowledge/30n-node-path-ensure.js',
    'modules/main/knowledge/30o-error-counts.js',
    'modules/main/knowledge/30p-path-query.js',
    'modules/main/knowledge/30q-entry-path-form.js',
    'modules/main/knowledge/30r-knowledge-cleanup.js',
    'modules/main/knowledge/30s-node-modal-utils.js',
    'modules/main/knowledge/30t-knowledge-tree-baseline-freeze.js',
    'modules/main/30-directory-management.js',
    'modules/main/31-inline-quiz.js',
    'modules/main/32-difficulty-rating.js',
    'modules/main/33-bulk-actions.js',
    'modules/main/34-modal-controls.js',
    'modules/main/36-tab-coordination.js',
    'modules/main/workspace/36e-view-switch.js',
    'modules/main/workspace/36f-note-chapter-actions.js',
    'modules/main/workspace/36g-note-editor-actions.js',
    'modules/main/workspace/36h-knowledge-workspace-state-utils.js',
    'modules/main/workspace/36i-knowledge-viewer-sync.js',
    'modules/main/workspace/36j-knowledge-note-preview-layout.js',
    'modules/main/workspace/36k-knowledge-directory-data-utils.js',
    'modules/main/workspace/36l-knowledge-error-navigation.js',
    'modules/main/workspace/36m-knowledge-note-editor-modal.js',
    'modules/main/workspace/36n-knowledge-workspace-shell-render.js',
    'modules/main/workspace/36o-knowledge-note-mode-render.js',
    'modules/main/workspace/36p-knowledge-workspace-events.js',
    'modules/main/modal/36q-knowledge-node-modal-open.js',
    'modules/main/modal/36r-knowledge-node-modal-submit.js',
    'modules/main/modal/36s-knowledge-node-dnd.js',
    'modules/main/modal/36t-knowledge-move-modal.js',
    'modules/main/modal/36u-knowledge-node-relink-and-reorder.js',
    'modules/main/workspace/36a-workspace-data-actions.js',
    'modules/main/workspace/36c-notes-view-helpers.js',
    'modules/main/workspace/36d-notes-tree-status.js',
    'modules/main/modal/36b-entry-ai-and-save.js',
    'modules/main/99-bootstrap.js',
    'modules/knowledge-workspace.js',
    'modules/knowledge-node-modal.js',
    'modules/data-management.js',
]

JS_VIEW_SPLIT_SOURCES = {
    'home': [
        'modules/main/00-event-dispatcher.js',
        'modules/main/state/01a-reason-utils.js',
        'modules/main/state/01b-error-workflow-utils.js',
        'modules/main/state/01c-knowledge-search-ui.js',
        'modules/main/01-state.js',
        'modules/main/02-indexeddb.js',
        'modules/main/03-storage-usage.js',
        'modules/main/04-utils.js',
        'modules/main/persistence/04a-startup-summary.js',
        'modules/main/persistence/04b-persist-primitives.js',
        'modules/main/persistence/05e-sync-snapshots.js',
        'modules/main/persistence/05f-portable-backup.js',
        'modules/main/persistence/05g-cloud-fetch.js',
        'modules/main/persistence/05h-cloud-restore-flow.js',
        'modules/main/persistence/05i-cloud-ui.js',
        'modules/main/persistence/05j-cloud-entry-actions.js',
        'modules/main/persistence/05k-cloud-save-actions.js',
        'modules/main/persistence/05l-cloud-schedule-actions.js',
        'modules/main/persistence/05m-sync-op-queue.js',
        'modules/main/persistence/05n-sync-cursor.js',
        'modules/main/persistence/05o-load-workspace-data.js',
        'modules/main/persistence/05p-backup-payload.js',
        'modules/main/persistence/05q-apply-remote-ops.js',
        'modules/main/05-persistence.js',
        'modules/main/persistence/05a-note-sync-and-history.js',
        'modules/main/persistence/05b-cloud-bootstrap-and-schedule.js',
        'modules/main/persistence/05c-cloud-restore-origin-utils.js',
        'modules/main/persistence/05d-local-backup-ui.js',
        'modules/main/08-initial-data.js',
        'modules/main/09-markdown.js',
        'modules/main/12-review-engine.js',
        'modules/main/14-sidebar-render.js',
        'modules/main/15-filters.js',
        'modules/main/21-dashboard-modules.js',
        'modules/main/22-history.js',
        'modules/main/24-type-detection.js',
        'modules/main/workspace/36e-view-switch.js',
        'modules/main/workspace/36f-note-chapter-actions.js',
        'modules/main/workspace/36g-note-editor-actions.js',
        'modules/main/workspace/36h-knowledge-workspace-state-utils.js',
        'modules/main/workspace/36i-knowledge-viewer-sync.js',
        'modules/main/workspace/36j-knowledge-note-preview-layout.js',
        'modules/main/workspace/36k-knowledge-directory-data-utils.js',
        'modules/main/workspace/36l-knowledge-error-navigation.js',
        'modules/main/workspace/36m-knowledge-note-editor-modal.js',
        'modules/main/workspace/36n-knowledge-workspace-shell-render.js',
        'modules/main/workspace/36o-knowledge-note-mode-render.js',
        'modules/main/workspace/36p-knowledge-workspace-events.js',
        'modules/main/36-tab-coordination.js',
    ],
    'workspace': [
        'modules/main/10-notes-panel.js',
        'modules/main/11-typed-note-tree.js',
        'modules/main/16-main-render.js',
        'modules/main/workspace/16a-workspace-actions.js',
        'modules/main/workspace/16b-workspace-list-render.js',
        'modules/main/17-error-card-render.js',
        'modules/main/workspace/17a-error-card-actions.js',
        'modules/main/27-backup-restore.js',
        'modules/main/knowledge/30a-knowledge-tree-state.js',
        'modules/main/knowledge/30b-knowledge-tree-render.js',
        'modules/main/knowledge/30c-knowledge-tree-actions.js',
        'modules/main/knowledge/30d-knowledge-tree-core.js',
        'modules/main/knowledge/30e-knowledge-tree-modal-flow.js',
        'modules/main/knowledge/30f-knowledge-tree-binding.js',
        'modules/main/knowledge/30g-knowledge-tree-dnd.js',
        'modules/main/knowledge/30u-knowledge-tree-sort-modal.js',
        'modules/main/knowledge/30h-walk-sync-notes.js',
        'modules/main/knowledge/30i-note-reference-utils.js',
        'modules/main/knowledge/30j-duplicate-merge.js',
        'modules/main/knowledge/30k-path-title-utils.js',
        'modules/main/knowledge/30l-node-collect-and-expand.js',
        'modules/main/knowledge/30m-legacy-path-normalization.js',
        'modules/main/knowledge/30n-node-path-ensure.js',
        'modules/main/knowledge/30o-error-counts.js',
        'modules/main/knowledge/30p-path-query.js',
        'modules/main/knowledge/30q-entry-path-form.js',
        'modules/main/knowledge/30r-knowledge-cleanup.js',
        'modules/main/knowledge/30s-node-modal-utils.js',
        'modules/main/knowledge/30t-knowledge-tree-baseline-freeze.js',
        'modules/main/30-directory-management.js',
        'modules/main/workspace/36a-workspace-data-actions.js',
        'modules/main/workspace/36c-notes-view-helpers.js',
        'modules/main/workspace/36d-notes-tree-status.js',
        'modules/main/workspace/36h-knowledge-workspace-state-utils.js',
        'modules/main/workspace/36i-knowledge-viewer-sync.js',
        'modules/main/workspace/36j-knowledge-note-preview-layout.js',
        'modules/main/workspace/36k-knowledge-directory-data-utils.js',
        'modules/main/workspace/36l-knowledge-error-navigation.js',
        'modules/main/workspace/36m-knowledge-note-editor-modal.js',
        'modules/main/workspace/36n-knowledge-workspace-shell-render.js',
        'modules/main/workspace/36o-knowledge-note-mode-render.js',
        'modules/main/workspace/36p-knowledge-workspace-events.js',
        'modules/knowledge-workspace.js',
        'modules/data-management.js',
    ],
    'modal': [
        'modules/main/06-ai-workbench.js',
        'modules/main/image/07a-image-value-utils.js',
        'modules/main/image/07b-ocr-question-ui.js',
        'modules/main/image/07c-ocr-question-runner.js',
        'modules/main/image/07d-global-search-preview.js',
        'modules/main/image/07e-process-image-modal.js',
        'modules/main/image/07f-global-search-modal-and-filters.js',
        'modules/main/07-image-processing.js',
        'modules/main/18-crud-modal.js',
        'modules/main/19-import-export.js',
        'modules/main/modal/19a-import-export-core.js',
        'modules/main/modal/19b-import-parse.js',
        'modules/main/modal/19c-import-normalize.js',
        'modules/main/modal/19d-import-merge.js',
        'modules/main/modal/19e-entry-flow-ui.js',
        'modules/main/20-claude-helper.js',
        'modules/main/23-md-toolbar.js',
        'modules/main/25-quick-print.js',
        'modules/main/26-export-upgrade.js',
        'modules/main/28-vocabulary-bank.js',
        'modules/main/29-quiz-shortcuts.js',
        'modules/main/modal/13b-quiz-pause-timer.js',
        'modules/main/modal/13c-quiz-queue-entry.js',
        'modules/main/modal/13d-quiz-session-shell.js',
        'modules/main/modal/13e-quiz-submit-flow.js',
        'modules/main/modal/13f-quiz-render-answer.js',
        'modules/main/modal/13g-quiz-review-render.js',
        'modules/main/modal/13h-random-note-candidates.js',
        'modules/main/modal/13i-random-note-practice-priority.js',
        'modules/main/modal/13j-random-note-modal-render.js',
        'modules/main/modal/13k-random-note-actions.js',
    'modules/main/modal/13l-random-question.js',
        'modules/main/13-quiz-flow.js',
        'modules/main/modal/13a-quiz-canvas-feedback.js',
        'modules/main/31-inline-quiz.js',
        'modules/main/32-difficulty-rating.js',
        'modules/main/33-bulk-actions.js',
        'modules/main/34-modal-controls.js',
        'modules/main/modal/36b-entry-ai-and-save.js',
        'modules/main/modal/36q-knowledge-node-modal-open.js',
        'modules/main/modal/36r-knowledge-node-modal-submit.js',
        'modules/main/modal/36s-knowledge-node-dnd.js',
        'modules/main/modal/36t-knowledge-move-modal.js',
        'modules/main/modal/36u-knowledge-node-relink-and-reorder.js',
        'modules/knowledge-node-modal.js',
    ],
    'bootstrap': [
        'modules/main/99-bootstrap.js',
    ],
}

# 约束治理：体积阈值告警（字节）
VIEW_BUNDLE_SIZE_WARNINGS = {
    'home': 280_000,
    'workspace': 180_000,
    'modal': 220_000,
    'bootstrap': 10_000,
}

VIEW_BUNDLE_LINE_WARNINGS = {
    'home': 7000,
    'workspace': 4500,
    'modal': 5000,
    'bootstrap': 300,
}

# 约束治理：重复函数名扫描白名单
DUPLICATE_FUNCTION_NAME_ALLOWLIST = {
}

# 约束治理：仅扫描主线拆分范围，避免兼容层噪音
DUPLICATE_FUNCTION_SCAN_PREFIXES = (
    'modules/main/',
)

SOURCE_FILE_LINE_WARNING = 500
