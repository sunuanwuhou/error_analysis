...
function renderRandomNoteReview() {
  ...
  const whyText = `距上次编辑 ${_formatGapDays(item.editGapDays)}，距上次查看 ${_formatGapDays(item.viewGapDays)}`;
  ...
  body.innerHTML = `
    <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:10px;flex-wrap:wrap">
      ...
      <div style="font-size:12px;color:#888">最后编辑：${escapeHtml(_formatIsoTime(item.updatedAt))} · 上次查看：${escapeHtml(item.lastViewedAt ? _formatIsoTime(item.lastViewedAt) : '从未')}</div>
    </div>
