...
from backend.runtime import build_runtime_label, infer_request_origin, normalize_origin, read_tunnel_url, request_is_secure
...
from backend.security import clear_session, create_user_account, get_user_by_token, issue_session, parse_iso_datetime, utcnow, verify_password
from backend.services.origin_status_service import list_origin_statuses, upsert_origin_status
...
def write_practice_log(user_id: str, payload: PracticeLogPayload) -> dict[str, Any]:
    from backend.services.practice_log_service import write_practice_log as _write_practice_log

    return _write_practice_log(user_id, payload)

def read_recent_practice_logs(user_id: str, limit: int = 30) -> list[dict[str, Any]]:
    from backend.services.practice_log_service import read_recent_practice_logs as _read_recent_practice_logs

    return _read_recent_practice_logs(user_id, limit)
