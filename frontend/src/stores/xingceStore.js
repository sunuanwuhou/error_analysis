...
const NOISY_ROOT_REMAP = new Map([
    ['片段阅读', '言语理解与表达'],
    ['数字推理', '数量关系'],
    ['数学运算', '数量关系'],
    ['和差倍比', '数量关系'],
...
function normalizeKnowledgeTitleForCleanup(title) {
    return String(title || '')
        .replace(/\uFEFF/g, '')
        .replace(/\u200B/g, '')
        .replace(/\u00A0/g, '')
...
function resolveNoisyRootAlias(title) {
    return NOISY_ROOT_REMAP.get(normalizeKnowledgeTitleForCleanup(title)) || '';
}
function makeVirtualRoot(title, order) {
    return {
...
function normalizeDisplayTitle(raw, fallback) {
    ...
    if (/^\?+$/.test(cleaned))
        return fallback;
    if (/^(?:\?{2,}|未分类\?|未细分\?|知识点\?)$/i.test(cleaned))
        return fallback;
    return cleaned;
}
function getNodeMergedIds(node) {
    const merged = node._mergedIds;
    if (!Array.isArray(merged) || merged.length === 0)
        return [node.id];
    return merged;
}
function mergeDuplicateSiblings(nodes) {
    if (!nodes.length)
        return;
    ...
    nodes.forEach(node => {
        ...
        for (const child of node.children ?? []) {
            ...
            if (normalizedChild && normalizedChild === normalizedSelf) {
                const mergedIds = new Set([...getNodeMergedIds(node), ...getNodeMergedIds(child)]);
...
function rebalanceDepth1ByAlias(roots) {
...
function buildTree(flatNodes) {
    ...
    for (const raw of flatNodes) {
        ...
        if (!id || !title)
            continue;
        if (INVALID_NODE_TITLES.has(title.toLowerCase()))
            continue;
    ...
    const sortChildren = (nodes, isRoot = false) => {
        nodes.sort((a, b) => {
            if (isRoot) {
            ...
            const ao = Number.isFinite(a.order) ? Number(a.order) : null;
            const bo = Number.isFinite(b.order) ? Number(b.order) : null;
...
function collectDescendantIds(node) {
    const ids = [...getNodeMergedIds(node)];
...
/** 在树中查找节点路径标题 */
function findNodePathTitles(nodes, id, parentTitles = []) {
    for (const n of nodes) {
...
function isVirtualKnowledgeNodeId(id) {
...
export const useXingceStore = defineStore('xingce', () => {
    ...
    // ── 练习统计缓存 ─────────────────────────────────────────────────────────────
    const practiceSummaries = ref({});
    let _summaryLoadTimer = null;
    const _pendingSummaryIds = new Set();
    // ── 练习面板状态 ─────────────────────────────────────────────────────────────
    ...
    /** 可参与「全量练习」的题量（未掌握类） */
    const eligibleFullPracticeCount = computed(() => errors.value.filter(e => e.status !== 'mastered' && e.masteryLevel !== 'mastered').length);
    // ── 筛选状态 ────────────────────────────────────────────────────────────────
    ...
    const errorSortOrder = ref('desc');
    // ── 知识树 UI 状态 ──────────────────────────────────────────────────────────
    const knowledgeExpandedIds = ref(new Set());
    ...
    const knowledgeSearchTerms = computed(() => String(knowledgeTreeSearch.value || '')
        .toLowerCase()
        .trim()
        .split(/\s+/)
        .filter(Boolean));
    ...
    function getNodePathText(nodeId) {
        if (!nodeId)
            return '';
        const titles = findNodePathTitles(knowledgeTree.value, nodeId) ?? [];
        return titles.join(' ');
    }
    function isNodeSearchMatch(node) {
    ...
    function isNodeVisibleBySearch(node) {
        if (!hasKnowledgeSearch())
            return true;
        if (isNodeSearchMatch(node))
            return true;
    ...
    function buildErrorSearchText(e) {
        const rec = e;
    ...
    function sortFilteredErrorsList(list) {
    ...
    function toggleErrorSortOrder(explicit) {
    ...
    const activeFilterCrumbs = computed(() => {
        ...
        if (statusFilter.value !== 'all') {
            const sm = { focus: '重点复习', review: '待复习', mastered: '已掌握' };
        ...
        if (sq)
            crumbs.push({ key: 'search', label: `"${sq}"` });
        return crumbs;
    });
    const filteredErrors = computed(() => {
        ...
        if (taskFilter.value !== 'all') {
            list = list.filter(e => {
                ...
                if (taskFilter.value === 'diagnose')
                    return stage === 'captured' || stage === 'diagnosing';
                if (taskFilter.value === 'review_ready')
                    return stage === 'review_ready';
                if (taskFilter.value === 'retrain')
    ...
    const taskCounts = computed(() => {
        ...
        for (const e of errors.value) {
            const stage = String(e.workflowStage ?? '');
            if (stage === 'captured' || stage === 'diagnosing')
                counts.diagnose++;
            else if (stage === 'review_ready')
                counts.review_ready++;
            else if (stage === 'retrain_due')
                counts.retrain++;
        }
        return counts;
    });
    ...
    function updateKnowledgeNode(id, patch) {
        knowledgeNodes.value = knowledgeNodes.value.map(n => n.id === id
            ? { ...n, ...patch, updatedAt: new Date().toISOString() }
            : n);
        scheduleSave();
    }
    /** 对齐旧版知识点重命名：同级重名校验 */
    function renameKnowledgeNode(nodeId, newTitle) {
        ...
        if (siblings.some(s => s.title === t)) {
            if (!window.confirm(`同级下已存在「${t}」，仍要重命名吗？`))
                return;
        }
    ...
    function batchApplyNoteNode(noteNodeId) {
        const ids = new Set(batchSelectedIds.value);
        if (!ids.size)
            return;
        const nid = noteNodeId || undefined;
        errors.value = errors.value.map(e => ids.has(e.id)
    ...
    function deleteKnowledgeNode(nodeId) {
        ...
        if (childCount > 0 || directErrors > 0) {
            window.alert(`不能直接删除「${node.title}」：还有 ${childCount} 个下级、${directErrors} 道直属题目。请先移动或清理后再删。`);
            return;
        }
        const rec = node;
        ...
        if (isRoot) {
            if (!window.confirm(`你正在删除一级知识点「${node.title}」。\n\n这会影响整组知识树结构和挂载错题，请再次确认。`)) {
                return;
            }
        }
    ...
    function canMoveKnowledgeNode(nodeId) {
        ...
        if (!node)
            return false;
        if (isVirtualKnowledgeNodeId(node.id))
            return false;
        const isRoot = !node.parentId;
        if (!isRoot)
            return true;
    ...
    function getKnowledgeMoveTargetOptions(nodeId) {
    ...
    function moveKnowledgeNode(nodeId, targetParentId) {
        ...
        const duplicate = knowledgeNodes.value.find(n => n.id !== node.id &&
    ...
    function globalSearchMatchError(e, terms) {
        if (!terms.length)
            return false;
        const text = buildErrorSearchText(e);
        return terms.every(t => text.includes(t));
    }
    function globalSearchMatchKnowledgeNode(n, terms) {
        if (!terms.length)
            return false;
    ...
    function toggleReasonFilter(reason) {
        taskFilter.value = 'all';
        activeNodeId.value = null;
        reasonFilter.value = reasonFilter.value === reason ? null : reason;
    }
    /** 侧栏 breadcrumb 单项移除 */
    function removeFilterCrumb(key) {
        if (key === 'node') {
            activeNodeId.value = null;
            return;
        }
    ...
    function expandKnowledgeNode(id) {
        if (!knowledgeExpandedIds.value.has(id)) {
    ...
    function loadKnowledgeExpandedState() {
        try {
            ...
            if (!Array.isArray(ids))
                return;
            knowledgeExpandedIds.value = new Set(ids.map(v => String(v || '').trim()).filter(Boolean));
        }
        catch {
    ...
    function saveKnowledgeExpandedState() {
        try {
            localStorage.setItem(KNOWLEDGE_EXPANDED_STORAGE_KEY, JSON.stringify([...knowledgeExpandedIds.value]));
        }
        catch {
    ...
    function clearErrorsByFilter(ids) {
        if (!ids.length)
            return;
    ...
    function replaceWorkspaceSnapshot(nextErrors, nextNodes) {
        errors.value = nextErrors;
        knowledgeNodes.value = nextNodes;
        scheduleSave();
    }
    /** 批量加载练习统计，只请求缓存中没有的 id，防抖 80ms */
    function queuePracticeSummaries(ids) {
    ...
    function invalidatePracticeSummaries(ids) {
        if (!ids?.length) {
    ...
    function clearActiveKnowledgeNote() {
        ...
        if (!confirm('清空当前知识点的笔记内容？此操作不可撤销。'))
            return;
