...
function normalizeSegmentsInPlace(segs) {
    for (const s of segs) {
        if (s.my_segment_summary === undefined)
            s.my_segment_summary = '';
    }
}
/** 选填的粉笔等机构参考答案，附在复制给 AI 的提示词末尾 */
function buildFenbiPromptSuffix(fenbi) {
    const t = fenbi.trim();
    if (!t)
        return '';
    return [
...
export const useShenlunStore = defineStore('shenlun', () => {
    const selectedNodeId = ref(SL_DEFAULT_NODE_ID);
    ...
    async function loadAttemptSummaries(sourceId) {
    ...
    async function autosave() {
        if (!questionText.value.trim() && !materialText.value.trim())
            return;
        try {
    ...
    async function formatMaterial() {
        ...
        try {
            ...
            attemptError.value = e.message;
        }
        finally {
            attemptLoading.value = false;
        }
    }
    function updateExtraction(index, text) {
        if (!attempt.value)
            return;
    ...
    function updateFinalSummary(text) {
        if (!attempt.value)
            return;
    ...
    async function saveAttemptProgress() {
        if (!attempt.value || attempt.value.id.startsWith('local-'))
            return;
        try {
    ...
    async function generateCCPrompt() {
        ...
        try {
            ...
            if (attempt.value.id.startsWith('local-')) {
                ccPromptText.value = _buildLocalPrompt(questionText.value, attempt.value.segments, attempt.value.my_final_summary);
            }
            else {
    ...
    async function submitCCPaste() {
        ...
        try {
            ...
            phase.value = 'done';
            if (sourceRecord.value?.id)
                await loadAttemptSummaries(sourceRecord.value.id);
            return updated.id;
        }
        catch (e) {
            ccPasteError.value = e.message;
            return null;
        }
        finally {
    ...
    async function patchWorkbenchNode(nodeId) {
        if (!sourceRecord.value?.id)
            return;
        try {
            sourceLoading.value = true;
    ...
    async function createNewAIRound() {
        if (!sourceRecord.value?.id || attempt.value?.id.startsWith('local-'))
            return false;
        const summaries = attemptSummaries.value;
        const latestCcStatus = summaries.length > 0 ? summaries[0].cc_status : attempt.value?.cc_status ?? '';
        if (!latestCcStatus)
            return false;
    ...
    async function deleteAttemptRecord(attemptId) {
    ...
    async function bootstrapFromRoute(query) {
...
function _buildLocalPrompt(question, segments, finalSummary) {
    const lines = [
    ...
    lines.push('【用户最终总结】', finalSummary.trim() || '（未填写）', '', '══════════════════════', '请返回纯 JSON，不要任何代码块标记；所有字符串值内如需引号请用中文直角引号「」，不要使用英文双引号嵌套。格式如下：', '', '{', '  "segments": [', '    {', '      "segment_index": 0,', '      "reference_extraction": "参考提炼（要点换行分隔）",', '      "matched_points": ["命中的要点"],', '      "missed_points": ["遗漏的要点"],', '      "wrong_points": ["有误的表述"],', `      "issue_tags": ["从以下选择：${_ISSUE_TAGS}"],`, '      "cc_comment": "简短点评 1-2 句"', '    }', '  ],', '  "reference_final_summary": "参考总结答案",', '  "overall_comment": "整体点评（100 字内）",', '  "overall_issue_tags": ["同上选项"]', '}', '', `共 ${segments.length} 个段落，segments 数组下标从 0 开始，与上面段落一一对应。`, '只返回 JSON，不要任何其他文字。');
    return lines.join('\n');
}
